	...
	const search = () => {
	    ...
	    if (query == "") {
	        ...
	        fetch(url)
	            ...
	            .then((data) => {
	                console.log(data);
	                // Show search results after receiving data
	                
	                let text=`<div class='list-group'`;
	                data.forEach((contact)=>
	                {
						text+=`<a href='/user/${contact.cId}/contact' class='lsit-group-item list-group-item-action'>${contact.name}</a>`
					});
					text+=`</div>`
	...
	
	{
		$.ajax({
			...
			success:function(response)
			{
				...
				swal("Failed !!","Your payment is successfull,but we did not get on server,we will contcat you as soon as possible","error");
			},
